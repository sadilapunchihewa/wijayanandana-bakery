...
import { deleteMeal, getMeals, mealCategories, updateMeal } from '../../services/adminService'
...
export default function ManageMeals() {
  ...
  const filtered = useMemo(() => {
    return meals.filter((meal) => {
      const matchesSearch = [meal.mealName, meal.description, meal.category]
        .join(' ')
        .toLowerCase()
    ...
    <div className="space-y-6">
      <section className="rounded-[34px] border border-white/70 bg-[#fffaf2]/92 p-6 shadow-[0_24px_70px_rgba(0,0,0,0.14)]">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#D97706]">Meal inventory</p>
            <h1 className="mt-2 font-display text-5xl font-extrabold text-[#1F2937]">Manage meals</h1>
          </div>
          <div className="flex flex-col gap-3 sm:flex-row">
            <input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1) }} placeholder="Search meals..." className={inputClass} />
            <select value={category} onChange={(event) => { setCategory(event.target.value); setPage(1) }} className={inputClass}>
              <option>All</option>
              {mealCategories.map((item) => <option key={item}>{item}</option>)}
            </select>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {visible.map((meal, index) => (
          <motion.article key={meal._id} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.04 }} whileHover={{ y: -8 }} className="overflow-hidden rounded-[32px] border border-white/70 bg-[#fffaf2]/92 shadow-[0_24px_70px_rgba(0,0,0,0.14)]">
            <div className="relative h-56 bg-gradient-to-br from-[#fed7aa] to-[#D97706]">
              {meal.image ? (
                <img src={meal.image} alt={meal.mealName} className="h-full w-full object-cover" />
              ) : (
                <div className="flex h-full items-center justify-center font-display text-4xl font-extrabold text-white/70">{meal.mealName?.[0] || 'W'}</div>
              )}
              <span className="absolute left-4 top-4 rounded-full bg-[#fffaf2]/86 px-3 py-1 text-xs font-bold text-[#7c2d12] backdrop-blur">{meal.category}</span>
              <span className={`absolute right-4 top-4 rounded-full px-3 py-1 text-xs font-bold ${meal.available ? 'bg-[#dcfce7] text-[#166534]' : 'bg-[#fee2e2] text-[#991b1b]'}`}>
            ...
            <div className="p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="font-display text-2xl font-extrabold text-[#1F2937]">{meal.mealName}</h2>
                  <p className="mt-1 text-sm text-[#7b5c49]">Rating {Number(meal.rating || 0).toFixed(1)}</p>
                </div>
                <p className="font-display text-xl font-extrabold text-[#D97706]">Rs {Number(meal.price).toLocaleString()}</p>
              </div>
              <p className="mt-4 line-clamp-2 text-sm leading-6 text-[#7b5c49]">{meal.description || 'No description added.'}</p>
              <div className="mt-5 flex gap-2">
                <button type="button" onClick={() => setEditing(meal)} className="flex flex-1 items-center justify-center gap-2 rounded-full border border-[#ead9bf] bg-white/70 px-4 py-2.5 text-sm font-semibold text-[#1F2937] hover:bg-white">
                  <Icon name="edit" className="h-4 w-4" /> Edit
                </button>
                <button type="button" onClick={() => setDeleteTarget(meal)} className="flex flex-1 items-center justify-center gap-2 rounded-full bg-[#1F2937] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#111827]">
      ...
      {!visible.length && <div className="rounded-[30px] border border-white/70 bg-[#fffaf2]/92 p-10 text-center text-[#7b5c49]">No meals found. Add your first Breakfast, Lunch or Dinner item.</div>}
      ...
      <AnimatePresence>
        {deleteTarget && (
          <Modal title="Delete meal?" onClose={() => setDeleteTarget(null)}>
            <p className="text-[#7b5c49]">This will remove <span className="font-semibold text-[#1F2937]">{deleteTarget.mealName}</span> from the meal section.</p>
        ...
        {editing && (
          <Modal title="Edit meal" onClose={() => setEditing(null)}>
            <form onSubmit={saveEdit} className="space-y-4">
              <input value={editing.mealName} onChange={(e) => setEditing({ ...editing, mealName: e.target.value })} className={inputClass} />
