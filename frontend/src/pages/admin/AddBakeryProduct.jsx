...
import { bakeryCategories, createBakeryProduct } from '../../services/adminService'
...
export default function AddBakeryProduct() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setForm(emptyForm)
      setPreview('')
      setMessage('Bakery product added successfully.')
    } catch (error) {
      setMessage(error.message || 'Unable to add bakery product.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-6 xl:grid-cols-[0.82fr_1.18fr] xl:items-start">
      <motion.section initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="rounded-[34px] border border-[#ead9bf] bg-[#fffaf2]/96 p-6 shadow-[0_24px_70px_rgba(0,0,0,0.14)]">
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#D97706]">New bakery product</p>
        <h1 className="mt-2 font-display text-5xl font-extrabold text-[#1F2937]">Add bakery item</h1>
        <p className="mt-3 text-sm leading-6 text-[#7b5c49]">
          Manage breads, buns, cakes, pastries, snacks and beverages with premium product images.
        </p>

        <label onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); handleImage(event.dataTransfer.files?.[0]) }} className="mt-8 flex min-h-[300px] cursor-pointer flex-col items-center justify-center overflow-hidden rounded-[30px] border border-dashed border-[#F59E0B]/70 bg-gradient-to-br from-[#fff8ec] to-[#fed7aa] p-5 text-center transition hover:border-[#D97706] xl:min-h-[420px]">
          {preview ? (
            <img src={preview} alt="Bakery product preview" className="h-full max-h-[320px] w-full rounded-[24px] object-cover shadow-[0_22px_60px_rgba(58,34,26,0.18)]" />
          ) : (
            <>
              <span className="flex h-16 w-16 items-center justify-center rounded-full bg-[#1F2937] text-3xl text-white">+</span>
              <span className="mt-5 font-display text-3xl font-extrabold text-[#1F2937]">Drop bakery image</span>
      ...
      <motion.section initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }} className="rounded-[34px] border border-[#ead9bf] bg-[#fffaf2]/96 p-6 shadow-[0_24px_70px_rgba(0,0,0,0.14)]">
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Product name">
            <input required value={form.productName} onChange={(e) => updateField('productName', e.target.value)} className={inputClass} placeholder="Ribbon Cake" />
          </Field>
          <Field label="Category">
            <select value={form.category} onChange={(e) => updateField('category', e.target.value)} className={inputClass}>
              {bakeryCategories.map((category) => <option key={category}>{category}</option>)}
            </select>
          </Field>
          <Field label="Price">
            <input required type="number" min="0" value={form.price} onChange={(e) => updateField('price', e.target.value)} className={inputClass} placeholder="2250" />
          </Field>
          <Field label="Rating">
        ...
        <Field label="Description" className="mt-4">
          <textarea value={form.description} onChange={(e) => updateField('description', e.target.value)} className={`${inputClass} min-h-36 resize-none py-4`} placeholder="Describe flavor, texture and serving style." />
        </Field>
        ...
        <motion.button whileHover={{ y: -2, scale: 1.01 }} whileTap={{ scale: 0.98 }} disabled={submitting} className="mt-6 w-full rounded-full bg-gradient-to-r from-[#DC2626] via-[#D97706] to-[#F59E0B] px-8 py-4 text-sm font-bold uppercase tracking-[0.18em] text-white shadow-[0_18px_40px_rgba(217,119,6,0.3)] disabled:opacity-60">
          {submitting ? 'Uploading...' : 'Upload Bakery Item'}
        </motion.button>
      </motion.section>
    </form>
  )
